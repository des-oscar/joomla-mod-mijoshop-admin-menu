...
class generateHtmlStructureHelper extends ControllerCommonColumnLeft
{

    public function generateHtml($menuData)
    {
        $htmlString = '<ul id="menu" class="nav mj-admin-menu';
        $htmlString .= '">';
        $htmlString .= '<li class="dropdown';
        $htmlString .= '"><a class="dropdown-toggle" data-toggle="dropdown" href="#">Mijoshop<span class="caret"></span></a>';
        $htmlString .= '<ul class="dropdown-menu scroll-menu">';

        foreach ($menuData['menus'] as $items) {
            if ($items['children']) {
                $htmlString .= ' <li class="dropdown-submenu"><a class="dropdown-toggle" data-toggle="dropdown" href="#">' . $items['name'] . '</a>';
                $htmlString .= $this->generateHtmlChildren($items['children']);
            } else {
                $htmlString .= ' <li><a class="no-dropdown ' . $items['id'] . '" href="' . $items['href'] . '">' . $items['name'] . '</a></li>';
            }
        };
    ...
    public function generateHtmlChildren($data)
    {
        $htmlChildrenString = '<ul class="dropdown-menu scroll-menu">';
        foreach ($data as $items) {
            if ($items['children']) {
                $htmlChildrenString .= ' <li class="dropdown-submenu"><a class="dropdown-toggle menu-' . strtolower($items['name']) . '" data-toggle="dropdown" href="#">' . $items['name'] . '</a>';
                $htmlChildrenString .= $this->generateHtmlChildren($items['children']);
            } else {
                $htmlChildrenString .= ' <li><a class="no-dropdown" href="' . $items['href'] . '">' . $items['name'] . '</a></li>';
            }
        };
        $htmlChildrenString .= '</ul>';
        return $htmlChildrenString;
    }
    ...
    public function index()
    {
        if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ((string)$this->request->get['user_token'] == $this->session->data['user_token'])) {
            ...
            $this->load->language('common/mijoshop');

            if ($version_status == 0) {
                $data['text_version_info'] = $this->language->get('text_version_latest');
                $data['version_class'] = "";
            } elseif ($version_status == -1) {
                $data['text_version_info'] = $this->language->get('text_version_old');
                $data['version_class'] = "version-old";
            } else {
                $data['text_version_info'] = $this->language->get('text_version_unknown');
